...
fn main() {
    ...
    for i in 0..q {
        let v = read_vec::<usize>();
        let (l, r, d) = (v[0] - 1, v[1] - 1, v[2] as i64);
        queries.push((l, r, d));
    }

    let mut seg: LazySegtree<MapMonoidB> = vec![(Modulo(1), Modulo(10)); n].into();

    for (l, r, d) in queries {
        seg.apply_range(l, r + 1, d);
        let ans = seg.prod(0, n).0;
        println!("{}", ans);
    }
}

type S = (Modulo, Modulo);
struct MonoidA;
impl Monoid for MonoidA {
    type S = S;
    fn identity() -> S {
        (Modulo(0), Modulo(1))
    }
